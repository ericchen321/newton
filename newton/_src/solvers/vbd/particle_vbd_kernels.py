...
# Positive-J guard telemetry uses one stable slot per reason and tetrahedron.
# Keep these values in one place so host decoding and device writes share the
# same deterministic layout.
POSITIVE_J_GUARD_REASON_INVALID_INPUT = 0
POSITIVE_J_GUARD_REASON_LEGACY_FLOOR_LOCK = 1
...
def reset_positive_j_guard_telemetry(
    ...
    if slot < event_ordinal.shape[0]:
        ...
        event_floor[slot] = 0.0
        event_proposed_determinant[slot] = 0.0
        ...
        event_local_vertex_order[slot] = -1
        event_x0[slot] = wp.vec3(0.0)
        event_displacement_before[slot] = wp.vec3(0.0)
        event_delta[slot] = wp.vec3(0.0)
        event_proposal[slot] = wp.vec3(0.0)
        event_pre_applied_particle[slot] = wp.vec3(0.0)
        event_final_applied_particle[slot] = wp.vec3(0.0)
        event_actual_post_application_particle[slot] = wp.vec3(0.0)
        event_rest_determinant[slot] = 0.0
        for vertex in range(4):
            event_tet_vertex_ids[slot, vertex] = -1
            event_current_tet_positions[slot, vertex] = wp.vec3(0.0)
...
@wp.kernel
def capture_positive_j_guard_actual_post_application(
    particle_q: wp.array[wp.vec3],
    event_ordinal: wp.array[wp.int32],
    event_site: wp.array[wp.int32],
    event_particle_id: wp.array[wp.int32],
    ...
    """Capture the particle position after the guarded path writes particle_q."""
    slot = wp.tid()
    site = event_site[slot]
    particle_id = event_particle_id[slot]
    if (
        site >= 0
        and site < 3
        and particle_id >= 0
        and particle_id < particle_count
        and event_pass_ordinal[slot] == pass_ordinal
    ):
        event_actual_post_application_particle[slot] = particle_q[particle_id]


@wp.func
def _record_positive_j_guard_event(
    reason: int,
    site: int,
    particle_index: wp.int32,
    tet_index: wp.int32,
    current_determinant: float,
    ...
    event_pre_postcheck_nonlegacy_alpha: wp.array[float],
    event_pre_postcheck_applied_determinant: wp.array[float],
    local_vertex_order: wp.int32,
    tet_vertex_ids: wp.vec4i,
    x0: wp.vec3,
    displacement_before: wp.vec3,
    delta: wp.vec3,
    proposal: wp.vec3,
    pre_applied_particle: wp.vec3,
    final_applied_particle: wp.vec3,
    current_tet_position0: wp.vec3,
    current_tet_position1: wp.vec3,
    current_tet_position2: wp.vec3,
    current_tet_position3: wp.vec3,
    rest_determinant: float,
    event_local_vertex_order: wp.array[wp.int32],
    event_tet_vertex_ids: wp.array2d[wp.int32],
    event_x0: wp.array[wp.vec3],
    event_displacement_before: wp.array[wp.vec3],
    ...
    if previous > ordinal:
        ...
        event_floor[slot] = determinant_floor
        event_proposed_determinant[slot] = proposed_determinant
        ...
        event_local_vertex_order[slot] = local_vertex_order
        event_tet_vertex_ids[slot, 0] = tet_vertex_ids[0]
        event_tet_vertex_ids[slot, 1] = tet_vertex_ids[1]
        event_tet_vertex_ids[slot, 2] = tet_vertex_ids[2]
        event_tet_vertex_ids[slot, 3] = tet_vertex_ids[3]
        event_x0[slot] = x0
        event_displacement_before[slot] = displacement_before
        event_delta[slot] = delta
        event_proposal[slot] = proposal
        event_pre_applied_particle[slot] = pre_applied_particle
        event_final_applied_particle[slot] = final_applied_particle
        event_actual_post_application_particle[slot] = final_applied_particle
        event_current_tet_positions[slot, 0] = current_tet_position0
        event_current_tet_positions[slot, 1] = current_tet_position1
        event_current_tet_positions[slot, 2] = current_tet_position2
        event_current_tet_positions[slot, 3] = current_tet_position3
        event_rest_determinant[slot] = rest_determinant
...
def _guard_vbd_displacement_increment_extended(
    ...
    for adjacent_tet in range(get_vertex_num_adjacent_tets(particle_adjacency, particle_index)):
        ...
        if (
            ...
            if proposed_determinant < determinant_floor:
                ...
                if denominator > 0.0 and wp.isfinite(denominator):
                    ...
                    bound = boundary_safety_factor * wp.clamp(alpha_boundary, 0.0, 1.0)
                    alpha = wp.min(alpha, bound)
                    nonlegacy_alpha = wp.min(nonlegacy_alpha, bound)
                else:
                    alpha = 0.0
                    nonlegacy_alpha = 0.0
        else:
    ...
    if telemetry_enabled or active_set_cycle_enabled:
        ...
        for adjacent_tet in range(get_vertex_num_adjacent_tets(particle_adjacency, particle_index)):
            ...
            if (
                ...
                if policy_code == 0:
                    reason = POSITIVE_J_GUARD_REASON_LEGACY_FLOOR_LOCK
                elif proposed_determinant < current_determinant:
                    reason = POSITIVE_J_GUARD_REASON_RECOVERY_DECREASE
                else:
                    reason = POSITIVE_J_GUARD_REASON_RECOVERY_NONWORSENING
            ...
            if reason >= 0 and (telemetry_enabled or active_set_cycle_enabled):
                _record_positive_j_guard_event(
                    ...
                    vertex_order,
                    wp.vec4i(i0, i1, i2, i3),
                    x0,
                    displacement_before,
                    delta,
                    x1,
                    pre_applied_particle,
                    final_applied_particle,
                    pos[i0],
                    pos[i1],
                    pos[i2],
                    pos[i3],
